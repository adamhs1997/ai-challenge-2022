...
with open("testData.csv", 'r') as f:
    test = np.array(list(map(lambda s: list(map(int, s.strip().split(","))), f.readlines())))
...
def classify_with_pca(classifier, n_components):
    ...
    train_data_transformed = pca.fit_transform(train, labels)
    test_data_transformed = pca.transform(test)

    classifier.fit(train_data_transformed, labels)
...
predictions = classify_with_pca(RandomForestClassifier(n_estimators=250), 50)
with open("predictions.csv", 'w') as outf:
    for p in predictions:
        print(p, file=outf)

# cross_validate(RandomForestClassifier(n_estimators=250), 50)
